...
async fn get_user(State(state): State<AppState>, Path(id): Path<i64>) -> Result<Json<Option<User>>, String> {
    ...
    let user = repo.find_by_id(&id).await.map_err(|e| format!("find_by_id: {e:#}"))?;
    Ok(Json(user))
}

// GET /users?email=foo@example.com
async fn list_user_by_email(State(state): State<AppState>, axum::extract::Query(params): axum::extract::Query<std::collections::HashMap<String, String>>)
    -> Result<Json<Vec<User>>, String>
{
    ...
    let repo: users_repo::Repository<UserRowAdapter> = users_repo::Repository::from_url_with_adapter(&state.db_url, UserRowAdapter)
        .await
        .map_err(|e| format!("repo: {e:#}"))?;
    let users = repo.find_by_email(&email).await.map_err(|e| format!("find_by_email: {e:#}"))?;
    Ok(Json(users))
}

// POST /users (executed inside a transaction using the LibsqlTransactionManager)
async fn create_user_tx(State(state): State<AppState>, Json(payload): Json<CreateUser>) -> Result<Json<User>, String> {
